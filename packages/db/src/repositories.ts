...
export class DrizzleActivityRepository implements ActivityRepository {
  ...
  async append(event: Parameters<ActivityRepository["append"]>[0]) {
    const [row] = await this.db.insert(activityEvents).values({
      ...event,
      scoreAtPublish: event.scoreAtPublish != null ? String(event.scoreAtPublish) : null,
    }).returning();
    if (!row) {
...
export class DrizzleRankingRepository implements RankingRepository {
  ...
  async upsert(input: {
    ...
    const existing = await this.findByOwnerAndBook({ ownerId: input.ownerId, bookId: input.bookId });
  ...
  async startBucket(input: {
    ...
    const existing = await this.findByOwnerAndBook({ ownerId: input.ownerId, bookId: input.bookId });
    if (!existing) throw new Error("Ranking not found");
    const newVersion = existing.version + 1;

    const [row] = await this.db
      .update(rankings)
      .set({ bucket: input.bucket, version: newVersion, updatedAt: new Date() })
      .where(
        and(
          eq(rankings.profileId, input.ownerId),
          eq(rankings.bookId, input.bookId),
          eq(rankings.version, existing.version)
        )
      )
      .returning();
    if (!row) throw new Error("Stale ranking version or ranking not found");
    return toRanking(row);
  }
}
  ...
  async upsertHashes(input: {
    ...
    await this.db
      ...
      .onConflictDoUpdate({
        ...
        set: {
          saltVersion: contactsIndex.saltVersion,
          expiresAt: contactsIndex.expiresAt,
        }
      });
  }
...
export class DrizzleListRepository implements ListRepository {
  ...
  async findById(id: EntityId, viewerId?: EntityId) {
    const row = await this.db.query.shelves.findFirst({
  ...
  async listByOwner(ownerId: EntityId, viewerId?: EntityId) {
    const isOwner = viewerId === ownerId;
    const rows = await this.db
      .select()
      .from(shelves)
      .where(
        isOwner
          ? and(eq(shelves.ownerId, ownerId), eq(shelves.kind, "list"))
          : and(
              eq(shelves.ownerId, ownerId),
              eq(shelves.kind, "list"),
              eq(shelves.visibility, "public")
            )
      );
    return rows.map((row) => ({
      id: row.id,
  ...
  async addItem(input: { listId: EntityId; bookId: EntityId; position: number }) {
    ...
    return {
      ...
      listId: row.shelfId,
      bookId: row.bookId,
      position: row.position ?? input.position,
      addedAt: row.addedAt,
    };
  }
