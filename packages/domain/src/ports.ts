...
export interface HandleHistoryRepository {
  record(input: { profileId: EntityId; oldHandle: string; retiredAt: Date; expiresAt: Date }): Promise<HandleHistory>;
  findCurrentHandleByOldHandle(oldHandle: string): Promise<string | null>;
  deleteExpired(): Promise<void>;
}
