...
describe("ranking.compare", () => {
  ...
  it("rerank compare flow publishes book_ranked activity event on completion", async () => {
    ...
    const repos = makeRepositories({
      ...
      activity: { append: vi.fn().mockResolvedValue({ id: "evt-1", actorId: UUID1, verb: "book_ranked", bookId: NEW_BOOK_ID, visibility: "followers", occurredAt: NOW, scoreAtPublish: 10, scoreLockedAtPublish: true }), getFriendFeed: vi.fn(), getFriendFeedGrouped: vi.fn(), deleteByReviewId: vi.fn(), listByActor: vi.fn().mockResolvedValue([]) },
    });
    const cache = makeCache(store);
    const app = buildApp(makeIdentity(), repos, cache);
